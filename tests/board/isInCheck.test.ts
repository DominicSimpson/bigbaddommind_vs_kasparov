...
describe('horizontal check detection', () => {
    ...
    it('detects that white queen on a8 does not check black king on d8 if there is a black bishop on c8', () => {
        const board = new ChessBoard();
        board.loadFEN('Q1bk4/8/8/8/8/8/8/3K4 b - - 0 1');
        expect(board.isInCheck('black')).toBe(false);
    });
    it('detects that a white queen on a8 checks a black king on d8', () => {
        const board = new ChessBoard();
        board.loadFEN('Q2k4/8/8/8/8/8/8/3K4 b - - 0 1');
        expect(board.isInCheck('black')).toBe(true);
    });
    it('detects that white queen on a7 does not check black king on d8', () => {
        const board = new ChessBoard();
        board.loadFEN('3k4/Q7/8/8/8/8/8/3K4 b - - 0 1');
        expect(board.isInCheck('black')).toBe(false);
    });


    it('detects that black queen on a1 does not check white king on d1 if there is white bishop on c1', () => {
        const board = new ChessBoard();
        board.loadFEN('3k4/8/8/8/8/8/8/q1bK4 w - - 0 1');
        expect(board.isInCheck('white')).toBe(false);
    });
    it('detects that a black queen on a1 checks a white king on d1', () => {
     ...
     it('detects that black queen on a2 does not check black king on d1', () => {
        const board = new ChessBoard();
        board.loadFEN('3k4/8/8/8/8/8/q7/3K4 w - - 0 1');
        expect(board.isInCheck('black')).toBe(false);
    });
